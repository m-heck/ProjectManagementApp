...
func TestDriver_UserInput(t *testing.T) {
	// Create a temporary directory for testing.
	tmpDir, err := ioutil.TempDir("", "driver-test")
	assert.NoError(t, err)
	defer os.RemoveAll(tmpDir)
...
func (d *Driver) Mutate(collection, resource string, v interface{}) error {
	if collection == "" {
		return fmt.Errorf("Missing collection  - no collection to change!")
	}
...
func (d *Driver) RemoveAll(collection, resource string) error {
	path := filepath.Join(collection, resource)
